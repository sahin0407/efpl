  ...
  return (
    <div className="pt-12 px-4 pb-12 w-full max-w-2xl mx-auto">
      <div className="mb-6 text-center sm:text-left border-b border-brand-border pb-4">
        <span className="text-[10px] text-brand-accent font-sans font-bold tracking-[0.25em] uppercase mb-1 block">TEAMS</span>
        ...
        {standings.map((stats, index) => {
          ...
          return (
            <motion.div
              ...
              className="bg-brand-surface rounded-sm border-y border-r border-l-[3px] border-l-transparent hover:border-l-brand-accent border-y-brand-border border-r-brand-border box-shadow-editorial relative overflow-hidden group transition-colors"
            >
              {/* Massive Watermark */}
              <div className="absolute -right-4 -bottom-10 pointer-events-none select-none z-0">
                <span className="font-mono font-bold text-[200px] text-brand-dark opacity-[0.06] leading-none">{club.shortName}</span>
              </div>
              
              <div className="p-6 relative z-10 w-full h-full">
                <div className="flex justify-between items-start mb-8">
                  <div className="flex gap-4 items-start">
                    <ClubBadge clubId={club.id} size={64} className="group-hover:shadow-[0_0_15px_rgba(200,168,75,0.3)]" />
                    <div>
                      <h3 className="font-playfair font-bold text-4xl text-brand-dark tracking-tight mb-3">{club.name}</h3>
                ...
                <div className="flex divide-x divide-brand-border border-t border-brand-border pt-4 mb-2">
                  <div className="flex-1 px-2 first:pl-0">
                    ...
                    <span className="font-mono text-2xl text-brand-dark font-bold">{stats.points}</span>
                  </div>
                  <div className="flex-1 px-4">
                    <span className="text-[9px] text-brand-muted font-sans font-bold uppercase tracking-widest block mb-1">GF</span>
                    <span className="font-mono text-2xl text-brand-dark font-medium">{stats.goalsFor}</span>
                  </div>
                  <div className="flex-1 px-4">
                    <span className="text-[9px] text-brand-muted font-sans font-bold uppercase tracking-widest block mb-1">GA</span>
                    <span className="font-mono text-2xl text-brand-dark font-medium">{stats.goalsAgainst}</span>
                  </div>
                  <div className="flex-1 px-4">
                    <span className="text-[9px] text-brand-muted font-sans font-bold uppercase tracking-widest block mb-1">CS</span>
                    <span className="font-mono text-2xl text-brand-dark font-medium">{stats.cleanSheets}</span>
                  </div>
                  <div className="flex-1 px-4">
                    <span className="text-[9px] text-brand-muted font-sans font-bold uppercase tracking-widest block mb-1">PENS</span>
                    <span className="font-mono text-2xl text-brand-dark font-medium whitespace-nowrap">{stats.penaltyWins}/{stats.penaltyLosses}</span>
                  </div>
                </div>

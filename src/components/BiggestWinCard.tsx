  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 mt-6 mb-6">
      <div className="bg-brand-surface border-t border-brand-accent shadow-[0_4px_20px_rgba(0,0,0,0.06)] py-6 px-4 text-center">
        <h3 className="text-brand-accent text-[10px] font-sans font-bold tracking-[0.25em] uppercase mb-4">BIGGEST WIN OF THE SEASON</h3>
        
        <div className="flex items-center justify-center gap-4">
          <span className="font-sans font-bold text-sm text-brand-dark flex-1 text-right">{getClub(biggestWinMatch.teamA).name}</span>
          <div className="flex items-center gap-3">
             <span className="font-mono text-2xl font-bold text-brand-dark">{biggestWinMatch.scoreA}</span>
             <span className="font-mono text-md text-brand-muted">-</span>
             <span className="font-mono text-2xl font-bold text-brand-dark">{biggestWinMatch.scoreB}</span>
          </div>
          <span className="font-sans font-bold text-sm text-brand-dark flex-1 text-left">{getClub(biggestWinMatch.teamB).name}</span>
        </div>
      </div>
    </div>
  );

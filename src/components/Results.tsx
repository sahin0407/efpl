        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex-1 flex items-center justify-center gap-2 sm:gap-4">
            <div className="flex-1 flex items-center justify-end gap-3">
              <span className={`font-sans text-sm hidden sm:inline ${aWon ? 'font-bold text-brand-text' : 'font-medium text-brand-gray'}`}>{teamA.name}</span>
              <span className={`font-sans text-sm sm:hidden ${aWon ? 'font-bold text-brand-text' : 'font-medium text-brand-gray'}`}>{teamA.shortName}</span>
              <ClubBadge clubId={match.teamA} size={32} />
              <div className="flex items-center gap-1">
                <span className={`font-mono text-lg font-bold w-6 text-center ${aWon ? 'text-brand-text' : 'text-brand-gray'}`}>{match.scoreA}</span>
                {hasPenalties && (
                  <span className={`text-[10px] sm:text-xs font-mono font-bold ${aWon ? 'text-cyber-purple' : 'text-brand-muted'}`}>({match.penaltyScoreA})</span>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 flex items-center justify-start gap-3">
              <div className="flex items-center gap-1">
                {hasPenalties && (
                  <span className={`text-[10px] sm:text-xs font-mono font-bold ${bWon ? 'text-cyber-purple' : 'text-brand-muted'}`}>({match.penaltyScoreB})</span>
                )}
                <span className={`font-mono text-lg font-bold w-6 text-center ${bWon ? 'text-brand-text' : 'text-brand-gray'}`}>{match.scoreB}</span>
              </div>
              <ClubBadge clubId={match.teamB} size={32} />
              <span className={`font-sans text-sm hidden sm:inline ${bWon ? 'font-bold text-brand-text' : 'font-medium text-brand-gray'}`}>{teamB.name}</span>
              <span className={`font-sans text-sm sm:hidden ${bWon ? 'font-bold text-brand-text' : 'font-medium text-brand-gray'}`}>{teamB.shortName}</span>
            </div>
          </div>
          ...
          <div className="w-24 text-right flex items-center justify-end gap-1.5 shrink-0 font-sans">
            ...
            <span className="inline-block bg-accent-gradient rounded-sm text-[9px] font-bold text-white px-2 py-0.5 uppercase tracking-widest">FT</span>
          </div>
        </div>
        
        {hasPenalties && (
          <div className="flex justify-center text-center -mt-1 mb-1 items-center gap-2">
            <span className="text-[10px] text-cyber-cyan font-sans font-bold italic">Won on penalties by {aWon ? teamA.shortName : teamB.shortName}</span>
          </div>
        )}
      </div>
    </motion.div>
  ...
  return (
    <div className="pt-12 px-4 pb-12 w-full max-w-2xl mx-auto">
      <div className="mb-6 text-center sm:text-left border-b border-brand-border pb-4">
        <span className="text-[10px] text-cyber-gold font-sans font-bold tracking-[0.25em] uppercase mb-1 block">HISTORY</span>

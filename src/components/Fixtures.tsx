...
const formatMatchDate = (dateStr?: string) => {
  ...
  if (diffDays === 0 || (d.getDate() === now.getDate() && diffDays < 0 && diffTime > -86400000)) {
    ...
    remainingClass = 'text-cyber-cyan';
  } else if (diffDays > 1) {
    remainingLabel = `In ${diffDays} days`;
    remainingClass = 'text-brand-success';
  }
    ...
    >
      ...
      <div className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 relative z-10`}>
        ...
        <div className="flex justify-between sm:flex-col sm:w-28 text-left">
          <span className="text-[10px] font-sans font-bold text-brand-muted uppercase tracking-wider">{stageNumber}</span>
        ...
        <div className="flex-1 flex items-center justify-center gap-4 py-2 sm:py-0">
          <div className="flex-1 flex items-center justify-end gap-3">
            <span className="font-sans font-bold text-sm text-brand-text hidden sm:inline">{teamA.name}</span>
            <span className="font-sans font-bold text-sm text-brand-text sm:hidden">{teamA.shortName}</span>
            {match.teamA !== 'tbd' && <ClubBadge clubId={match.teamA} size={40} />}
          </div>
          
          {match.isLive ? (
            <div className="flex items-center gap-2">
              <span className="font-mono text-2xl font-black text-brand-text">{match.scoreA ?? 0}</span>
              <span className="font-sans text-brand-gray text-lg">-</span>
              <span className="font-mono text-2xl font-black text-brand-text">{match.scoreB ?? 0}</span>
            </div>
          ) : (
            <span className="font-playfair italic text-sm text-gradient-primary font-bold">vs</span>
          )}
          
          <div className="flex-1 flex items-center justify-start gap-3">
            {match.teamB !== 'tbd' && <ClubBadge clubId={match.teamB} size={40} />}
            <span className="font-sans font-bold text-sm text-brand-text hidden sm:inline">{teamB.name}</span>
            <span className="font-sans font-bold text-sm text-brand-text sm:hidden">{teamB.shortName}</span>
          </div>
        </div>
        ...
        <div className="w-28 text-right flex items-center justify-end gap-1.5 shrink-0">
          {!isTBD && (
            <>
              ...
              <button
                ...
                className="p-1.5 hover:bg-brand-nav/20 rounded-full transition-all focus:outline-none focus:ring-1 focus:ring-cyber-purple"
                title={isNotified ? 'Unsubscribe from alerts' : 'Notify me when live'}
              >
                <Bell 
                  size={14} 
                  className={`transition-all ${isNotified ? 'text-cyber-purple fill-cyber-purple scale-110' : 'text-brand-gray/60 hover:text-cyber-purple'}`} 
                />
              </button>
            </>
          )}
          {match.isLive ? (
            ...
            <span className={`inline-block border ${isTBD ? 'border-[#D0D0D0] text-[#A0A0A0]' : 'border-cyber-purple text-brand-muted bg-brand-border'} rounded-sm text-[9px] font-sans font-bold px-2 py-0.5 uppercase tracking-widest`}>
  ...
  return (
    <div className="pt-12 px-4 pb-12 w-full max-w-2xl mx-auto">
      <div className="mb-6 text-center sm:text-left border-b border-brand-border pb-4">
        <span className="text-[10px] text-cyber-pink font-sans font-bold tracking-[0.25em] uppercase mb-1 block">CALENDAR</span>
        ...
        <p className="text-[11px] text-brand-gray font-sans mt-2 tracking-wide">
          Tap the bell icon <Bell size={10} className="inline mx-0.5 text-cyber-purple fill-cyber-purple" /> to receive local browser push notifications when your favorite clubs play LIVE matches.
        </p>
      </div>
